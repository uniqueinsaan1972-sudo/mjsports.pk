...
export default function ProductDetailView({ product }) {
  const { name, willow, price, old, description, specs, grad, thumbnail } = product;
  ...
  const image = (className) =>
    thumbnail ? (
      <img src={thumbnail} alt={name} className={className} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
    ) : (
  ...
  return (
    <div className="pd-layout">
      <div className="pd-gallery">
        <div className="pd-main-image" style={{ background: `linear-gradient(160deg, ${grad[0]}, ${grad[1]})` }}>
          {image()}
        </div>
        <div className="pd-thumbs">
          {[0, 1, 2].map((i) => (
            <div key={i} className="pd-thumb" style={{ background: `linear-gradient(160deg, ${grad[0]}, ${grad[1]})` }}>
              {image()}
            </div>
          ))}
        </div>
      ...
      <div className="pd-info">
        ...
        <div className="pd-action-row">
          ...
          <button className="btn-primary pd-add-btn" onClick={() => addItem(product, qty, weight)}>
            Add to Cart
          </button>
        </div>
        ...
        <a href={getInquiryWhatsappLink(product)} target="_blank" rel="noopener noreferrer" className="pd-whatsapp-link">
