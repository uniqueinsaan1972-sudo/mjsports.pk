...
export async function addProduct({ title, description, price, discount, category, images, featured }) {
...
export async function updateProduct(id, { title, description, price, discount, category, images, featured }) {
  const pricing = computePricing(price, discount);
  const imageList = images && images.length ? images : [];
